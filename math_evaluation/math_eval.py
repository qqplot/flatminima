...
def worker_process(rank, world_size, args, data_name, examples_chunk):
    ...
    if args.use_vllm:
        llm = LLM(
            ...
            # tokenizer_mode는 경로가 아니라 "auto" 또는 "slow"여야 합니다.
            tokenizer_mode="auto", 
            # 만약 RoPE 에러가 계속된다면 아래 주석을 해제하세요.
            # max_model_len=8192, 
        )
        tokenizer = None
        if args.apply_chat_template:
            # [수정 포인트 2] 토크나이저 로드 시 발생할 수 있는 'list' 에러 방지
            tokenizer = AutoTokenizer.from_pretrained(
                args.model_name_or_path, 
                trust_remote_code=True,
                # 일부 환경에서 fast tokenizer 에러 시 아래 옵션을 사용해볼 수 있습니다.
                # use_fast=False 
            )
    else:
