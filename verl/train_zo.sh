...
#SBATCH --output=logs/numina-cot-dft_zo-%j.out             # 표준 출력 로그 (logs 폴더 미리 생성 필요)
#SBATCH --error=logs/numina-cot-dft_zo-%j.err 
...
experiment_name="numina-cot-dft_zo-qwen-2.5-instruct-3b--lr-${lr}-wd-${weight_decay}"
save_path=checkpoints/$experiment_name

torchrun --standalone --nnodes=1 --nproc_per_node=$nproc_per_node \
        -m verl.trainer.fsdp_zo_trainer_dft \
