...
# 1. 기본 자원 설정
#SBATCH --job-name=dft_adazo      # 작업 이름
...
#SBATCH --time=72:00:00                  # 최대 실행 시간 (1일)
#SBATCH --output=logs/numina-cot-dft_adazo-%j.out             # 표준 출력 로그 (logs 폴더 미리 생성 필요)
#SBATCH --error=logs/numina-cot-dft_adazo-%j.err 
...
experiment_name="numina-cot-dft_adazo-qwen-2.5-instruct-3b--lr-${lr}-sam-rho-${sam_rho_max}-sam-rho-min-${sam_rho_min}-zo-sigma-${zo_sigma}-wd-${weight_decay}"
save_path=checkpoints/$experiment_name

torchrun --standalone --nnodes=1 --nproc_per_node=$nproc_per_node \
        -m verl.trainer.fsdp_adazo_trainer_dft \
